...
## Load merge file
## Assign colors
clusterMergeFile = paste0(work,"/Config/merge wjh.xlsx") #create dummy merger numbers prior to annotation
cluster_merging <- read_excel(clusterMergeFile)
clusterlevels=c(1:40) ## Set cluster number
colorassigned<-kovesi.rainbow_bgyrm_35_85_c69(length(unique(cluster_merging$new_cluster)))
...
clusterMergeFile = paste0(work,"/Config/merge wjh.xlsx")
cluster_merging <- read_excel(clusterMergeFile)

clusterlevels=c("Gran",
                "Mac_I",
                "Mac_II",
                "Tc",
                "Th",
...
plot_clustering_heatmap_wrapper2(fcs=output$fcs,
                                 ...
                                 subtype_markers = c("CD45",
                                                     "CD15", "MPO", "CD68", "CD163", "HLADR", "CD206",
                                                     "CD8", "CD4", "FOXP3", "GZMB", "PD1", "CD20"),
                                color_clusters = colorassigned,
                                cell_clustering = factor(output$cell_clustering1m, levels=clusterlevels), 
                                fileName = 'Clusteringheatmap_merged.pdf')
dev.off()

## dot plots

exprdf <- as.data.frame(fsApply(output$fcs1,exprs))
exprdf$sample_id <- output$sample_ids
exprdf$cluster <- output$cell_clustering1m
colrep<-hue_pal()(length(clusterlevels)-1)

plot_list <- list()
for (i in 1:length(samplevels)){
  samp<-samplevels[i]
  exprsub <- exprdf[exprdf$sample_id==samp,]
  ggp_repimg <- ggplot(exprsub, aes(x=X_position, y=Y_position))+
    ggtitle(samp)+
    geom_point(alpha=0.9, aes(fill=cluster), color="darkgray", shape=21, stroke = 0.1, size=1)+
  ...
  plot_list[[i]]<-ggp_repimg
}

compiledplot <- ggarrange(plotlist = plot_list, ncol=1, nrow=1)
pdf('representative_dotplots.pdf',width=5,height=5)
compiledplot
dev.off()
...
## DENSITY
ggp<-ggplot(ggdfd,aes(x=Treatment,y=densities))+
  geom_boxplot(outlier.color=NA, lwd=0.25, fill=NA)+
  geom_jitter(width=0.1, size=1, aes(color=Treatment))+
...
ggp2<-ggplot(ggdfd,aes(x=Treatment,y=densities))+
  geom_boxplot(outlier.color=NA, lwd=0.25, fill=NA)+
  geom_jitter(width=0.1, size=1, aes(color=Treatment))+
  facet_wrap(~cluster,ncol=4,scales="free")+
  ylab("Density of Cells")+
...
pdf('Density_box.pdf',width=4.5,height=3.5)
ggp
ggp2
dev.off()
